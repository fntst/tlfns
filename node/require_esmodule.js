...
function getImportDirName() {
  ...
  // 去掉 node 模块加载 相关的栈调用 
  stackList = stackList.filter( itm=>(!itm.includes("Module.") && !itm.includes("require ")) )
  ...
  return pathMd.parse(stackList.slice(-1)[0].file).dir
}
let importDirname = getImportDirName();

module.exports = function(path){
  path = pathMd.resolve(importDirname, path)
...
/* 使用:
  const require_esmodule_file = require("../require_esmodule_file.js");
